...
public class ClassLoaderLookupPolicy
{
    ...
    /**
     * @return true if the given package name is defined as overridden, false otherwise
     */
    public boolean isOverridden(String packageName)
    {
        // find a match
        boolean overrideMatch = false;
        for (String override : overridden)
        {
            if (packageName.equals(override) || packageName.startsWith(override + "."))
            {
                overrideMatch = true;
                break;
            }
    ...
    public boolean isBlocked(String packageName)
    {
        ...
        {
            if (packageName.equals(b) || packageName.startsWith(b + "."))
            {
                blockedMatch = true;
                break;
            }
