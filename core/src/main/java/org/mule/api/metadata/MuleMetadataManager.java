...
public class MuleMetadataManager implements MetadataManager, MuleContextAware
{
    ...
    private MetadataAware findMetadataAwareExecutable(ProcessId processId) throws InvalidExecutableIdException
    {
        ...
        if (flow == null)
        {
            throw new InvalidExecutableIdException(String.format(PROCESSOR_NOT_FOUND, processId.getProcessPath()));
        }
        try
        {
            if (!processId.getProcessPath().equals("-1"))
            {
                try
                {
                    return ((MetadataAware) flow.getMessageProcessors().get(Integer.parseInt(processId.getProcessPath())));
                }
                catch (IndexOutOfBoundsException | NumberFormatException e)
                {
                    throw new InvalidExecutableIdException(String.format(PROCESSOR_NOT_FOUND, processId.getProcessPath()), e);
                }
            }
            else
            {
